...
def _wrap_caption(text: str, draw, font, max_width: int) -> list[str]:
    ...
    if not text:
        return [""]
    if _line_width(draw, text, font)[0] <= max_width:
        return [text]
    words = text.split(" ")
    if len(words) == 1:
        chars = list(text)
        ...
        for ch in chars:
            trial = line + ch
            if line and _line_width(draw, trial, font)[0] > max_width:
                lines.append(line)
                line = ch
            else:
                line = trial
    ...
    lines, cur = [], ""
    for word in words:
        trial = word if not cur else "%s %s" % (cur, word)
        if cur and _line_width(draw, trial, font)[0] > max_width:
            lines.append(cur)
            cur = word
        else:
            cur = trial
...
def write_caption_png(text: str, path: Path, width: int = 1080) -> None:
    ...
    max_text_w = width - 72
    lines = _wrap_caption(text, draw, font, max_text_w)
