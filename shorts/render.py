...
def _line_bbox(draw, line: str, font, stroke_width: int = 6) -> tuple[int, int, int]:
    ...
    return width, top or 0, bottom or 0
...
def write_caption_png(text: str, path: Path, width: int = 1080) -> None:
    ...
    sizes = [_line_bbox(draw, line, font, stroke) for line in lines]
    gap = 8
    text_h = sum(bottom - top for _w, top, bottom in sizes) + gap * (len(lines) - 1)
    pad_y = 34
    box_h = max(text_h + pad_y * 2, 128)
    img = Image.new("RGBA", (width, box_h), (0, 0, 0, 235))
    draw = ImageDraw.Draw(img)
    y = (box_h - text_h) // 2
    for line, (_lw, top, bottom) in zip(lines, sizes):
        tokens = [p for p in KEYWORD_RE.split(line) if p] or [line]
        tw, _, _ = _line_bbox(draw, line, font, stroke)
        x = (width - tw) // 2
        draw_y = y - top
        for tok in tokens:
            ...
            bbox = draw.textbbox((x, draw_y), tok, font=font, stroke_width=stroke)
            x = bbox[2]
        y += (bottom - top) + gap
    img.save(path)
