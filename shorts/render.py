...
def _wrap_caption_lines(text: str, draw, font, usable: int) -> list[str]:
    raw = " ".join((text or "").replace("\n", " ").split())
    if not raw:
        return [""]
    tw, _th = _line_width(draw, raw, font)
    if tw <= usable:
        return [raw]
    lines = textwrap.wrap(raw, width=12) or [raw]
    return lines[:2]
...
def write_caption_png(text: str, path: Path, width: int = 1080) -> None:
    ...
    pad_y = 32
    box_h = max(text_h + pad_y * 2, 120)
    scratch = Image.new("RGBA", (width, box_h + 80), (0, 0, 0, 0))
    sdraw = ImageDraw.Draw(scratch)
    y = 40
    for line, (_lw, lh) in zip(lines, sizes):
        tokens = [p for p in KEYWORD_RE.split(line) if p] or [line]
        tw, _ = _line_width(sdraw, line, font)
        x = (width - tw) // 2
        for tok in tokens:
            fill = GOLD if KEYWORD_RE.fullmatch(tok) else WHITE
            sdraw.text((x, y), tok, font=font, fill=fill, stroke_width=5, stroke_fill=(0, 0, 0, 240))
            bbox = sdraw.textbbox((x, y), tok, font=font, stroke_width=5)
            x = bbox[2]
        y += lh + gap
    ink = scratch.getbbox()
    bar = Image.new("RGBA", (width, box_h), (0, 0, 0, 255))
    if ink:
        cropped = scratch.crop(ink)
        cx = (width - cropped.width) // 2
        cy = (box_h - cropped.height) // 2
        bar.paste(cropped, (cx, cy), cropped)
    bar.save(path)
