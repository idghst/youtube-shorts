...
def _caption_lines(text: str, width: int, font, draw) -> list[str]:
    raw = text.replace("\n", " ").strip() or text
...
def write_caption_png(text: str, path: Path, width: int = 1080) -> None:
    ...
    lines = _caption_lines(text, width, font, draw)
    ...
    for line, (_lw, lh) in zip(lines, sizes):
        ...
        tw, _ = _line_width(ink, line, font)
        x = (width - tw) // 2
        for tok in tokens:
            fill = GOLD if KEYWORD_RE.fullmatch(tok) else WHITE
            ink.text((x, y), tok, font=font, fill=fill, stroke_width=5, stroke_fill=(0, 0, 0, 240))
            bbox = ink.textbbox((x, y), tok, font=font, stroke_width=5)
            x = bbox[2]
        y += lh + gap
    crop = canvas.getbbox()
    if crop is None:
        ink_img = Image.new("RGBA", (width, 80), (0, 0, 0, 0))
    else:
        ink_img = canvas.crop((0, crop[1], width, crop[3]))
    pad_y = 28
    box_h = ink_img.height + pad_y * 2
    img = Image.new("RGBA", (width, box_h), (0, 0, 0, 255))
    img.paste(ink_img, (0, (box_h - ink_img.height) // 2), ink_img)
    img.save(path)
