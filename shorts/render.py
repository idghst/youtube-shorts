...
def _wrap_caption(text: str, draw, font, max_width: int) -> list[str]:
    ...
    if not raw:
        return [""]
    full_w, _ = _line_width(draw, raw, font)
    if full_w <= max_width:
        return [raw]
    for limit in (16, 14, 12, 10, 8):
        lines = textwrap.wrap(raw, width=limit) or [raw]
        lines = lines[:2]
        if all(_line_width(draw, line, font)[0] <= max_width for line in lines):
            return lines
    return textwrap.wrap(raw, width=8)[:2] or [raw]
...
def write_caption_png(text: str, path: Path, width: int = 1080) -> None:
    ...
    for line, (_lw, lh) in zip(lines, sizes):
        ...
        cursor = 0
        for tok in tokens:
            bbox = draw.textbbox((cursor, 0), tok, font=font, stroke_width=5)
            ink_boxes.append(bbox)
            cursor = bbox[2]
        if ink_boxes:
            ink_left = min(b[0] for b in ink_boxes)
            ink_right = max(b[2] for b in ink_boxes)
            x = (width - (ink_right - ink_left)) // 2 - ink_left
        else:
            x = pad_x
        for tok in tokens:
            fill = GOLD if KEYWORD_RE.fullmatch(tok) else WHITE
            bbox = draw.textbbox((x, y), tok, font=font, stroke_width=5)
            draw.text((x, y), tok, font=font, fill=fill, stroke_width=5, stroke_fill=(0, 0, 0, 255))
            x = bbox[2]
        y += lh + gap
    img.save(path)
