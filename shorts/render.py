...
def _render_caption_line(line: str, font, width: int):
    from PIL import Image, ImageDraw

    tokens = [p for p in KEYWORD_RE.split(line) if p] or [line]
    strip = Image.new("RGBA", (width, 220), (0, 0, 0, 0))
    ...
    for tok in tokens:
        ...
        x = bbox[2]
    ink = strip.getbbox()
    return strip.crop(ink) if ink else strip
...
def write_caption_png(text: str, path: Path, width: int = 1080) -> None:
    ...
    rendered = [_render_caption_line(line, font, width) for line in lines]
    gap = 8
    text_h = sum(im.height for im in rendered) + gap * (len(rendered) - 1)
    pad_y = 30
    img_h = max(text_h + pad_y * 2, 120)
