...
걱정하는 시니어가 나오는 재테크 쇼츠. **대본·이미지는 Cursor 에이전트**, 나머지는 로컬 CLI.
...
흐름: Supabase 이전 주제 → 선정 → 대본 → 제목 → 설명 → 해시태그 → 이미지 → 렌더 → 업로드 → Supabase 기록.

## 자막

대본 자막은 이해하기 쉽게. 장면마다 `captions` 2개 이상. 한 덩어리로 오래 띄우지 말고 문장 단위로 끊는다. 뉴스 용어는 쉬운 말로. 한 줄 28자 이하. 흰 글자 + 어두운 박스/외곽선, 하단 세이프 영역. 숫자·핵심어만 색/굵기. 키네틱 금지. 해요체·짧은 구어·질문. 습니다 연속·메타 자막 금지.
