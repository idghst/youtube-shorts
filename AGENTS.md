...
2. 에이전트가 대본 → 제목 → 설명 → 해시태그 순으로 `script.json`을 쓰고 `GenerateImage`로 `scene-01.png` … 작성. 이미지 프롬프트는 Grok Imagine 자연어 영어 2~3문장(9:16, 하단 1/3 비움, 걱정하는 시니어(사람) 필수). OpenAI/Gemini/FAL, imagegen CLI 금지. 해요체. 습니다 연속·메타 자막 금지.
