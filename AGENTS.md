...
2. 에이전트가 `style.anchor`와 `style.face`를 고정한 뒤 대본 → 제목 → 설명 → 해시태그 순으로 `script.json`을 쓴다. 장면 클립은 **Grok Imagine Video**로만 `scene-01.mp4`(5~10초, 9:16). GenerateImage/png 대체 금지. 첫 클립 프레임으로 나머지 얼굴을 고정. 한 쇼츠 안은 같은 얼굴·나이·화풍·이어지는 스토리. 장편 애니 톤. 실사 사람·망가체·줌 확대·검정 레터박스 금지. OpenAI/Gemini/FAL/Runway/Kling, imagegen CLI 금지. 해요체. 첫 자막은 훅, 마지막은 내 돈.
