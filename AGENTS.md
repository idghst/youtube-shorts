...
2. 에이전트가 `style.anchor`와 `style.face`를 고정한 뒤 대본 → 제목 → 설명 → 해시태그 순으로 `script.json`을 쓰고 GenerateImage로 `scene-01.png` … 작성. 동영상 클립은 만들지 않는다. 첫 컷을 레퍼런스로 얼굴을 고정. 한 쇼츠 안은 같은 얼굴·나이·화풍·이어지는 스토리. 장편 애니 톤. 실사 사람·망가체·줌 확대·검정 레터박스 금지. OpenAI/Gemini/FAL, imagegen CLI 금지. 해요체. 첫 자막은 훅, 마지막은 내 돈.
