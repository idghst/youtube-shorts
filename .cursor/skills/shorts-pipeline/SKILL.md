...
1. **이전 주제** + **주제 선정**: `.venv/bin/python -m shorts pick --channel 돈이웃` → stdout이 잡 폴더. **통장·이체·한도·예금보호·부모 전세**가 시니어 국가통계보다 앞. 시니어 관심(연금·노후·건보·상속)은 그다음. 은평·2030·MZ·국가 조 단위·연 N%만 있는 금리는 감점. 이미 올린 한도+숫자는 재탕으로 건너뛴다. 숫자·삭감·인상 훅이 있으면 가산. “쓸 헤드라인 없음”이면 중단.
...
4. **제목**: 대본을 본 뒤에 `title`. 대본 첫 줄을 옮기지 말 것. 내 돈 상황 + **억/만 원**. 팔지말지·팔면·지역·2030·만 명 제목 금지. 같은 한도+숫자 재탕 금지.
...
천만 조회수는 기획 목표이지 보장이 아니다. Studio(2026-08-20): 28일 조회 1.9만, Shorts 피드 98.0%, **이탈 68.7%**, 유효 조회 5.9천, 검색 0.8%. 시청 45세+ 86%, KR 59%. Shorts 도달 탭에 노출·클릭률은 없다. 훅은 첫 3초 이탈이다.

채널 최고는 [전세금 부모 무이자 2억](https://youtube.com/shorts/bWIrl9LT5Og) 2.4천, 49초, 평균시청 37초(77%), 피드 100%, 계속 시청 20.5%. 같은 패턴 1천+: 자녀 통장 5000만 세금, ISA 한도 2000만 소멸, 종부세 14억 vs 주택연금 12억, 예금보호 1억 합산. **같은 ISA·한도·2000만 재탕은 94회.** `내 집 팔면 양도세, 2억이 9억?`는 140회. `변동 주담대 이자, 연 6%?`는 86회. `전세 3만·월세 5만`은 172회(만 명·만 가구처럼 금액이 아님). **1:01 지역뉴스(은평·2030)는 조회 14.** `팔지 말지?`는 193회. 국가 500조는 295회. `30년 아파트 정전` 1천은 재테크 한도가 아니니 베끼지 마라. 그 편을 베끼되: **통장·이체·한도·세금 결과 + 억/만 원 + 45~49초. 같은 한도+숫자 금지.** 구독·댓글 CTA는 넣지 마라.
...
- **억, 만 원, 원, 또는 세금·한도 %가 숫자와 붙어야 한다.** `1000만 명` `3만` `연 6%`만으로는 안 된다. 2030·조 단위도 안 된다.
- 결과는 세금·사라짐·합산·한도까지. `팔지 말지?` `팔면` `은평으로?` `누가?` 금지. 시청 86%가 45세+라 2030·MZ 타깃 금지.
- 이미 올린 **같은 한도 주제+같은 숫자**(ISA 2000만, 전세금 2억)는 각도만 바꿔도 재탕이다.
- 국가 통계(퇴직연금 500조)보다 **내가 옮길 수 있는 한도**(무이자 2억, 통장 5000만)가 먹힌다. 금리 연 N%만 있는 주담대는 한 달 N만 원·세금 결과보다 약하다.
- 호기심·걱정. 물음표 있으면 더 좋다.
- 쉬운 말. 입니다/습니다 금지.
