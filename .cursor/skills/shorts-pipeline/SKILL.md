...
scenes 4~5개. 훅 + 사실 + 더 아픈 사실 + 내 돈. 장면 `duration`은 3초 배수, `beats` 개수 = duration/3. 합 48~60초. **권장 48초/16장.** 채널 최고 조회 편이 49초다. 60초는 이탈이 크면 쓰지 마라. 위 JSON은 장면 1만 예시. 그림은 같은 하루로 이어지되, **각 컷은 그 3초 자막의 사실을 보여준다.** 대본은 화면 자막만. `load_script`가 카피·화풍·메시지 검사를 한다.

천만 조회수는 기획 목표이지 보장이 아니다. 채널 최고는 [전세금 부모 무이자 2억](https://youtube.com/shorts/bWIrl9LT5Og) 약 2.4천, Shorts 피드 99%, 마지막까지 20%. 그 편을 베끼되 더 세게: 국가 단위 조 숫자 대신 **내 집 억/만**, 첫 3초에 제목 숫자, 커스텀 썸네일.

## 제목 (클릭·검색)

- 해시태그 넣지 말 것. 12~42자. 길면 줄여라.
- **주제가 한눈에.** `tags` 중 하나가 제목에 그대로 있어야 한다.
- **내 돈 상황**이 보여야 한다: 전세·부모·증여·이자·한도·건보·연금·월급.
- 국가 통계(가계빚 2000조)보다 **내가 할 수 있는 한도**(무이자 2억)가 먹힌다.
- 호기심·걱정·희망. **숫자 필수.** 물음표 있으면 더 좋다.
- 쉬운 말. 입니다/습니다 금지.
- 예: `전세금 부모에게 빌리면, 무이자 2억?`
...
1. 훅: **첫 자막에 제목 숫자를 그대로.** 질문으로. 제목 문장 복붙 금지. 3초 안에 한도가 보여야 한다.
