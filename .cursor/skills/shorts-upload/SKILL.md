...
2. 업로드 전 `.venv/bin/python -m shorts meta --dir <잡폴더>`. 출력의 title/description/hashtags/hashtag_chips/tags를 그대로 쓴다. 손으로 다시 치지 마라.
...
5. 썸네일: `thumb.png`를 커스텀 썸네일로 올린다. 자동 프레임만 두고 넘어가지 마라.
6. 아동용 아님 유지.
...
3. **해시태그** 칸에 `meta.hashtags`를 붙이거나 `meta.hashtag_chips`를 하나씩 넣는다. 칩이 실제로 생겼는지 센다. **5개 미만이면 `다음` 금지.** `#돈이웃` `#쇼츠` `#shorts`는 넣지 마라.
...
- 해시태그: 주제만 5~9개. `#돈이웃` `#쇼츠` `#shorts` 금지. **자세히 칸에 반드시 넣는다.**
